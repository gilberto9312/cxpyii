...
class SiteController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',
				'actions'=>array('index','fecha'),
				'users'=>array('@'),
			),
			array('allow',
				'actions'=>array('contact','login','logout','error','captcha','widget','extension','module','design','ajaxContent'),
				'users'=>array('*'),
			),
			array('allow',
	...
	public function getFecha()
	{
		...
		//variable de acceso
		$sql= "SELECT fecha,  SUM(total)from gastos GROUP BY fecha";
		...
		$datareader->bindColumn(1, $fecha );

		$datareader->bindcolumn(2, $total);
        ...
        while(($row=$dataReader->read())!==false) {
            $data[] = array($row['fecha']." (".$row['total'].")", intval($row['total']), 'stroke-color: #0050FF; stroke-opacity: 0.6; stroke-width: 2; fill-color: #76A7FA; fill-opacity: 0.2');
        }
