...
class GastosController extends Controller
{
	...
	public function accessRules()
	{
		...
		$datareader2=$connection->createCommand($sql)->query();
		...
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('grafico','fecha','index','view','alpha','batch','updateYears','suggestTrabajador'),
				'users'=>array('@'),
			),
	...
	{
		$model_detalleIngreso = new DetalleIngreso;
		if (isset($_POST['DetalleIngreso']))
		{
			$model_detalleIngreso->attributes=$_POST['DetalleIngreso'];
			$model_detalleIngreso->idingreso = $id;
			if($model_detalleIngreso->save())
				$this->redirect(array('view','id'=>$id));
		}
		$this->render('view',array(
			'model'=>$this->loadModel($id),
			'model_detalleIngreso'=>$model_detalleIngreso,
		));
	}
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Gastos']))
		{
			...
			if($model->save())
				$this->redirect(array('detalleIngreso/create','id'=>$model->idingreso));
	...
	{
		$dataProvider=new CActiveDataProvider('Gastos');
	 ...
	 * @param Gastos $model the model to be validated
	 */
