<?php
   ...
   * @copyright (c) 2013, Ing. Edgar Gonzalez
   * @version 1.0
   *
   * @license this is licensed under the terms of the Open Source
...
class Sesion {
 public function ValidarSesion()
 {
  ...
  $Datos = new Vector2d();
  $Datos->setV(0,1,$usuario);
  $Datos->setV(0,2,$clave);
  $valor = $objusuario->Buscar($Datos);
  if ($valor==1) {
   $_SESSION["rolsesion"] = $Datos->getV(0,2);
  }
  return $valor;
 }
...
class HandlerString {
 ...
 function searchWord($word,$pattern) {
  $value = 0;
  if (strlen(stripos($word, $pattern))>0) {
  	$value = 1;
  }
  return $value;	
 }
 function searchWordPosition($word,$pattern) {
  $value = 0;
  if (strlen(stripos($word, $pattern))>0) {
    $value = stripos($word, $pattern);
  }
  return $value;  
 }
 function searchPattern($word,$patterns) {
  $value = null;
  $separator = explode(",", $patterns);
  $i=0;
  while ($i<sizeof($separator)) {
   if ($this->searchWord($word,strtolower($separator[$i]))==1) {
    $value=strtolower($separator[$i]);
    $i=sizeof($separator); 		
   }
   $i++;
  }
 ...
 function removeCRLF($str, $sustiuir) {
